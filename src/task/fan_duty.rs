...
pub static FANDUTY_SIGNAL: signal::Signal<CriticalSectionRawMutex, u8> = signal::Signal::new();
...
pub fn init(
    ...
    static LSTIMER0: StaticCell<ledc::timer::Timer<ledc::LowSpeed>> = StaticCell::new();
    let lstimer0 = LSTIMER0.init_with(|| {
...
pub async fn fan_duty(pwm_channel: ledc::channel::Channel<'static, LowSpeed>) {
    loop {
        // Wait for a new duty cycle to be signalled.
        let new_fan_duty = FANDUTY_SIGNAL.wait().await;
