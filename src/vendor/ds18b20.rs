...
mod resolution {
    ...
    impl Resolution {
        ...
        pub async fn delay_for_measurement_time(&self, delay: &mut impl AsyncDelayNs) {
...
impl Ds18b20 {
    ...
    pub async fn save_to_eeprom<T, E>(
        &self,
        onewire: &mut OneWire<T>,
        blocking_delay: &mut impl BlockingDelayNs,
        async_delay: &mut impl AsyncDelayNs,
    ) -> OneWireResult<(), E>
    where
        T: InputPin<Error = E>,
        T: OutputPin<Error = E>,
    {
        save_to_eeprom(Some(&self.address), onewire, blocking_delay, async_delay).await
    }
...
where
    ...
    T: OutputPin<Error = E>,
{
    save_to_eeprom(None, onewire, blocking_delay, async_delay).await
}
...
where
    ...
    T: OutputPin<Error = E>,
{
    onewire.send_command(commands::COPY_SCRATCHPAD, address, blocking_delay)?;
    async_delay.delay_us(10000).await; // delay 10ms for the write to complete
    Ok(())
}
